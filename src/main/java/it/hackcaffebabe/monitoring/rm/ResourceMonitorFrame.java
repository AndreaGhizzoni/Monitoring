...
class ResourceMonitorFrame extends Frame
{
	...
	class PanelInfo extends Panel
	{
		...
		public void paint(Graphics g){
			super.paint( g );
			g.setFont( f );
			g.drawChars( this.title, 0, this.title.length, 0, 10 );
			g.drawChars( this.max, 0, this.max.length, 0, 25 );
			g.drawChars( this.tot, 0, this.tot.length, 0, 40 );
			g.drawChars( this.free, 0, this.free.length, 0, 55 );
		}
	}
